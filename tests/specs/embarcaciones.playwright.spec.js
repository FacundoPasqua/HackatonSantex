...
const ESPERA_ENTRE_LOTES = 3000; // 3 segundos de espera entre lotes
...
const isTest = BOT_URL.includes('test');
...
let BASE_SHEET_NAME = 'Embarcaciones';
if (isLocalhost) {
  BASE_SHEET_NAME = 'Embarcaciones_Local';
} else if (isPreprod) {
  BASE_SHEET_NAME = 'Embarcaciones_Preprod';
}else if (isTest){
  BASE_SHEET_NAME = 'Embarcaciones_Test';
}
...
function readQuestions() {
  ...
  return data
    ...
    .map((row, idx) => ({
      ...
      palabrasClave: String(row['Palabras clave'] || '').trim(),
      fuente: String(row['Subcategoría'] || '').trim(),
      añoVigencia: String(row['Tipo de prueba'] || '').trim()
    }))
...
if (preguntas.length === 0) {
  ...
  test.describe.serial('Embarcaciones - Procesamiento por Lotes', () => {
